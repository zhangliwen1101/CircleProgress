...
public class RoundProgressBar extends View {
	...
	public RoundProgressBar(Context context, AttributeSet attrs,
		...
		default_suffix_padding = dp2px(getResources(), 6);
		default_suffix_text = "%";
		default_bottom_text_size = sp2px(getResources(), 10);
		default_stroke_width = dp2px(getResources(), 5);// 进度条的宽度
	...
	protected void initByAttributes(TypedArray attributes) {
		...
		arcAngle = attributes.getFloat(R.styleable.ArcProgress_arc_angle,
				default_arc_angle);
	...
	protected void initPainters() {
		...
		paint = new Paint();
		paint.setColor(getResources().getColor(R.color.pro_gray));
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		rectF.set(strokeWidth / 2f + 5, strokeWidth / 2f + 5, width
				- strokeWidth / 2f - 5, MeasureSpec.getSize(heightMeasureSpec)
				- strokeWidth / 2f - 5);
	...
	protected void onDraw(Canvas canvas) {
		...
		System.out.println(TAG + startAngle + "       " + arcAngle);
		paint.setColor(finishedStrokeColor);
		canvas.drawArc(rectF, finishedStartAngle, finishedSweepAngle, false,
				paint);

		System.out.println(TAG + finishedStartAngle + "       "
				+ finishedSweepAngle);
		drawRedDot(canvas, finishedSweepAngle, getWidth() / 2, getHeight() / 2,
				getWidth() / 2f - strokeWidth + 2.5f);
		String text = String.valueOf(getProgress());
		if (!TextUtils.isEmpty(text)) {
			textPaint.setColor(textColor);
			textPaint.setTextSize(textSize);
			float textHeight = textPaint.descent() + textPaint.ascent();
			float textBaseline = (getHeight() - textHeight) / 2.0f - 80;
			...
			canvas.drawText(suffixText,
					getWidth() / 2.0f + textPaint.measureText(text)
							+ suffixTextPadding, textBaseline + textHeight + 50
